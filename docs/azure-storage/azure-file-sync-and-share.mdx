...
8. The file share has [snapshot](https://docs.microsoft.com/en-us/azure/storage/files/storage-snapshots-files) created, when you open the file share from Windows explorer, it will make "Previous Versions" is available when connecting directly to the Azure File Share. Right click any folder under `\\howdyfiles.file.core.windows.net\share`, choose "Previous Versions", it will list file/folder previous versions based on snapshot in Azure file share.
